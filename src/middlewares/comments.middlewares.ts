...
export const isOwnerOrAdminComments = async (req:Request, res:Response, next:NextFunction)=>{
    ...
    if(req.method == 'PATCH'){
        ...
        if(user?.type_user === 'admin' || comment?.user.id == userId){
            return next();
        }
...
export const commentExistsbyId = async (req: Request, res: Response, next: NextFunction) => {
